...
from app.infrastructure.postgres.tables import EvalTaskTable
...
class SqlAlchemyEvalTaskRepository(EvalTaskRepository):
    ...
    async def list_pending(self, *, limit: int) -> list[EvalTask]:
        ...
        rows = (
            await self._session.scalars(
                ...
                .where(
                    EvalTaskTable.status == EvalTaskStatus.PENDING,
                    or_(
    ...
    async def reset_for_retry(self, *, item_id: str) -> None:
        # 条件写：只复位 FAILED 的那一行；attempts 归零意味着退避预算重新满血，
        # next_retry_at 清空让 list_pending 立刻可见
        result = await self._session.execute(
            update(EvalTaskTable)
            .where(
                EvalTaskTable.item_id == item_id,
                EvalTaskTable.status == EvalTaskStatus.FAILED,
            )
            .values(
                status=EvalTaskStatus.PENDING,
                attempts=0,
