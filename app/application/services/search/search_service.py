...
框架无关：只依赖 ports（ChunkIndex / EmbeddingClient / Reranker），
不感知 Neo4j / httpx / 任何具体实现，也不引入检索框架。
...
class SearchService:
    ...
    def __init__(
        ...
        *,
        ...
        rerank_top_n: int = 20,
        graph: GraphRecallChannel | None = None,
    ) -> None:
        self._index = index
    ...
    async def search(self, query: str, *, top_k: int | None = None) -> list[dict]:
        ...
        if self._rewriter is not None:
            ...
            rewrite_entities = rewrite.entities
        ...
        # 图谱第三路：只递提案，任何异常降级成空列表（等价 graph 关闭）。
        # 原查询向量顺手递给实体链接做向量兜底——dense 路本来就要 embed，零成本
        graph_hits: list[dict] = []
        if self._graph is not None and dense_vectors:
            try:
                graph_hits = await self._graph.recall(
                    query, entities=rewrite_entities, query_vector=dense_vectors[0]
                )
            except Exception:
                logger.warning("图谱频道降级，本次检索按原两路继续", exc_info=True)
        # 并发查询 所有的rewrite query 和 关键词
        results = await asyncio.gather(
            *(
                self._index.search_by_dense(dense_vector=vector, top_k=candidate_limit)
                for vector in dense_vectors
            ),
            *(
                self._index.search_by_sparse(query_text=text, top_k=candidate_limit)
        ...
        channels = [*dense_results, *sparse_results]
        if graph_hits:
            channels.append(graph_hits)
        fused = rrf_fuse(channels, k=_RRF_K, top_k=candidate_limit)  # RRF
        if self._reranker is not None:
