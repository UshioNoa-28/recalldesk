...
输出不符合 schema 时由 LLM 客户端直接抛错——刻意不做静默回退，
...
- entities: 2-6 named entities (proper nouns/terms) mentioned or implied by the
  query, for knowledge-graph entity linking; keep them in the query's language

Output ONLY JSON:
{{"queries": ["...", "..."], "keywords": ["...", "..."], "entities": ["...", "..."]}}
...
class QueryRewriteOutput(BaseModel):
    ...
    entities: list[str] = Field(default_factory=list, description="图谱链接用实体名")
...
class QueryRewriteService:
    ...
    async def rewrite(self, query: str, *, n_variants: int = 3) -> QueryRewrite:
        ...
        entities = [e.strip() for e in out.entities if e and e.strip()]
        return QueryRewrite(queries=queries, keywords=keywords, entities=entities)
