...
开跑 = 同一事务里写 running 行 + 按 ready 题 fan-out 逐题任务（eval_run_tasks），
投递与检索都由后台进程（run publisher / run consumer）接手，这里只做起跑前的
判断和读：POST 立刻返回 run_id，客户端轮询详情接口看 progress_done / status。
"""
...
class EvalRunService:
    ...
    async def start_run(self, testset_id: str, *, top_k: int | None = None) -> dict:
        ...
        await self._runs.create_run(
            ...
            config={
                "top_k": k,
                "query_rewrite": self._search.rewrite_enabled,
                "graph": self._search.graph_enabled,
            },
        )
        for item in ready:
            await self._run_tasks.add(run_id=run_id, item_id=item.id)
        try:
            await self._unit_of_work.commit()
        except IntegrityError as exc:
            # _require_idle 的「查 running 再插入」之间有并发窗口，DB 里的
            # 部分唯一索引（一个测试集最多一个 running）是最终防线；
            # 在这里把冲突翻译成 400 给前端。run 行带任务行一起回滚，
            # 没有留下孤儿任务
            raise ValueError(
    ...
    async def get_run(self, run_id: str) -> dict:
        ...
        items = await self._runs.list_run_items(run_id)
        return {**_run_dict(run), "items": [_run_item_dict(item) for item in items]}

    async def rerun_failed(self, run_id: str) -> dict:
        """重跑失败 run 里的失败题：任务复位 + run 翻回 running，再走既有消费链路。

        已有结果行的题由 has_run_result 幂等跳过，不重复计；全部任务重新落定后
        finalize 基于全部结果行重算指标。同测试集有别的 running 时拒（start_run
        同款规矩，部分唯一索引兜并发窗口）。
        """
        ...
        if run is None:
            raise KeyError(run_id)
        if run.status is not EvalRunStatus.FAILED:
            raise ValueError("只有失败的运行可以重跑失败题")
        await self._require_idle(run.testset_id)
        if not await self._run_tasks.reset_failed_for_run(run_id):
            raise ValueError("这次运行没有可重跑的失败任务")
        await self._runs.reopen_run(run_id)
        try:
            await self._unit_of_work.commit()
        except IntegrityError as exc:
            raise ValueError("这个评测集已经有其它运行在跑") from exc
        return await self.get_run(run_id)
    ...
    async def list_runs(self, testset_id: str) -> dict:
        ...
        if await self._evals.get_testset(testset_id) is None:
            raise KeyError(testset_id)
        runs = await self._runs.list_runs(testset_id)
    ...
    async def _require_idle(self, testset_id: str) -> None:
        ...
        running = [
            run
            for run in await self._runs.list_runs(testset_id)
...
def _run_item_dict(item: EvalRunItem) -> dict:
    """逐题结果：recall / map / evidence_coverage 都是派生的，
    也一并给出去省得前端再算一遍口径。recall 是块级全在（0/1），
    与聚合 recall@k 同名同义；文档级宽口径已退役，low_recall 随之前端
    判色改用 recall==0。"""
    ...
    return {
        ...
        "evidence_ranks": item.evidence_ranks,
        "recall": item.joint_hit,
        "map": round(average_precision, 4) if average_precision is not None else None,
        "latency_ms": item.latency_ms,
        "evidence_total": item.evidence_total,
        "evidence_matched": item.evidence_matched,
        "evidence_coverage": round(item.evidence_coverage, 4),
    }
