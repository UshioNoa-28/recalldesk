...
run 也是异步的：POST .../runs 登记一次运行并按题 fan-out 评测任务就返回，
逐题检索由后台的 eval_run consumer 跑，进度与指标同样轮询 /eval/runs/{run_id}。
"""
...
from app.application.services.evaluation.eval_service import (
    MAX_EVIDENCE_CHUNKS_PER_ITEM,
    EvalService,
)
...
class EvidenceGroup(BaseModel):
    """一题的证据组：平权坐标列表，顺序只是拼装顺序，无主坐标；跨文档自由混合。"""

    chunks: list[SelectedChunk] = Field(
        ..., min_length=1, max_length=MAX_EVIDENCE_CHUNKS_PER_ITEM
    )
...
class AddItemsRequest(BaseModel):
    ...
    两种写法并存：`chunks` 是旧的「一块一题」，`groups` 是新的「一组一题
    （多证据）」；至少要给一种。
    """

    chunks: list[SelectedChunk] = Field(
        default_factory=list, max_length=MAX_CHUNKS_PER_REQUEST
    )
    ...
    def _require_something(self) -> AddItemsRequest:
        if not self.chunks and not self.groups:
            raise ValueError("chunks 与 groups 至少要给一个")
        return self
...
async def add_testset_items(
    ...
    不查重：同批勾选重复提交会如实地再出同样的题（防双击归前端）。
    多证据题走 groups：一组一块起步，第一块是答案所在，其余是补充素材。
    """
    ...
    try:
        return await service.add_items(testset_id, groups)
...
@router.post("/eval/testsets/{testset_id}/retry_failed_items")
@inject
async def retry_failed_items(
    testset_id: str,
    service: FromDishka[EvalService],
) -> dict:
    """重试集内全部出题失败的题目：条目与任务复位，后台重新生成。

    ready 的题与 run 历史不动；没有失败条目时是空操作。看结果仍然轮询详情接口。
    """

    try:
        return await service.retry_failed_items(testset_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="评测集不存在") from exc
...
@router.post("/eval/runs/{run_id}/rerun_failed")
@inject
async def rerun_failed_run(
    run_id: str,
    service: FromDishka[EvalRunService],
) -> dict:
    """重跑失败 run 的失败题：复位任务、run 回 running，后台续跑；轮询详情看进度。"""

    try:
        return await service.rerun_failed(run_id)
    except KeyError as exc:
