...
export const EvaluationStudio: React.FC<EvaluationStudioProps> = ({
  ...
  const [confirmDeleteSet, setConfirmDeleteSet] = useState<TestSet | null>(null);
  ...
  const handleRerunFailedRun = async (runId: string) => {
    setRerunningRunId(runId);
    try {
      const updated = await api.rerunEvalRun(runId);
      setSelectedRunDetail(updated);
      addToast('success', '已提交重跑', '失败题已复位，评测在后台继续跑');
      if (selectedTestSet) fetchRuns(selectedTestSet.id, true);
    } catch (err) {
      addToast('error', '重跑失败', formatErrorMessage(err));
    } finally {
  ...
  const executeDeleteTestSet = async (ts: TestSet) => {
    setDeletingId(ts.id);
    try {
  ...
  const handleRetryFailedItems = async (testsetId: string) => {
    setRetryingItems(true);
    try {
      const updated = await api.retryFailedTestSetItems(testsetId);
      addToast('success', '已重新提交出题任务', '失败题目已复位，后台 LLM Worker 正在重新生成');
      setSelectedTestSet(updated);
      await fetchTestSets();
    } catch (err) {
      addToast('error', '重试出题失败', formatErrorMessage(err));
    } finally {
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row min-h-0 bg-slate-50 overflow-hidden">
        ...
        {selectedTestSet ? (
          <div className="max-w-5xl space-y-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4 shadow-xs">
              {(() => {
                const failedItemsCount = (selectedTestSet.items || []).filter(
                  (i) => i.status === 'failed'
                ).length;
                  ...
                  <>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {(failedItemsCount > 0 || selectedTestSet.status === 'failed') && (
                          <button
                            onClick={() => handleRetryFailedItems(selectedTestSet.id)}
                            disabled={retryingItems}
                            className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-mono font-semibold text-amber-800 hover:text-amber-900 bg-amber-50 hover:bg-amber-100 border border-amber-300 rounded-md transition-colors cursor-pointer shadow-2xs disabled:opacity-50"
                            title="重试集内所有出题失败的题目"
                          >
                            <RotateCw
                              className={`w-3.5 h-3.5 text-amber-600 ${
                                retryingItems ? 'animate-spin' : ''
                              }`}
                            />
                            <span>重试失败出题 {failedItemsCount > 0 ? `(${failedItemsCount})` : ''}</span>
                          </button>
                        )}
                        ...
                        <button
                          onClick={() => setConfirmDeleteSet(selectedTestSet)}
                    ...
                    {selectedTestSet.error_message && (
                      <div className="p-3 bg-rose-50 border border-rose-200 rounded-lg text-xs font-mono text-rose-700 flex items-center justify-between gap-3">
                        ...
                        >
                          <RotateCw
                            className={`w-3 h-3 ${retryingItems ? 'animate-spin' : ''}`}
                          />
                          <span>重试失败题目</span>
                        </button>
                      </div>
            ...
            {activeTab === 'items' ? (
              <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4 shadow-xs">
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-mono">
                  ...
                  <div className="flex items-center gap-2">
                    {(() => {
                      const failedCount = (selectedTestSet.items || []).filter(
                        (i) => i.status === 'failed'
                      ).length;
                      ...
                      return (
                        <button
                          onClick={() => handleRetryFailedItems(selectedTestSet.id)}
                          disabled={retryingItems}
                          className="flex items-center gap-1.5 px-2 py-0.5 text-[11px] font-mono font-semibold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-300 rounded-md transition-colors cursor-pointer disabled:opacity-50"
                          title={`重试集内 ${failedCount} 道出题失败的题目`}
                        >
                          <RotateCw
                        ...
                        >
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-slate-200/80 text-xs font-mono">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <span className="text-slate-400 text-[11px]">
                                {evidenceList.length > 0 ? `共 ${evidenceList.length} 块证据` : '暂无证据'}
                              </span>
                            </div>
                          </div>
                          ...
                          {evidenceList.length > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5 text-xs font-mono">
                              <span className="text-slate-400 font-medium text-[11px] mr-0.5">考查证据:</span>
                              {evidenceList.map((ref, eIdx) => {
                                const evDocName = docMap[ref.document_id] || `文档 ${ref.document_id.substring(0, 8)}...`;
                                return (
                                  <div
                                    key={`${ref.document_id}:${ref.chunk_index}`}
                                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[10px] bg-white border-slate-200 text-slate-700"
                                  >
                                    <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-purple-100 text-purple-800">
                                      证据 {eIdx + 1}
                                    </span>
                                    <span className="truncate max-w-[140px]" title={evDocName}>
                                      {evDocName}
                                    </span>
                                    <span className="text-blue-700 font-bold">#chunk_{ref.chunk_index + 1}</span>
                                    {onNavigateToChunk && (
                                      <button
                                        type="button"
                                        onClick={() =>
                                          onNavigateToChunk({
                                            ...
                                            documentName: evDocName,
                                          })
                                        }
                                        className="hover:text-blue-600 text-slate-400 p-0.5 cursor-pointer ml-0.5"
                                        title="在文档浏览器中查看该切片"
                                      >
                          ...
                          {item.query ? (
                            ...
                            <div className="p-3 bg-rose-50 border border-rose-200 rounded-lg text-xs font-mono text-rose-700 flex items-center justify-between gap-3">
                              <div>⚠️ 出题失败: {item.error_message || '未知错误'}</div>
                              <button
                                onClick={() => handleRetryFailedItems(selectedTestSet.id)}
                                disabled={retryingItems}
                                className="shrink-0 flex items-center gap-1 px-2.5 py-1 bg-rose-100 hover:bg-rose-200 active:bg-rose-300 text-rose-800 rounded text-[11px] font-semibold transition-colors cursor-pointer disabled:opacity-50"
                              >
                                <RotateCw
                ...
                {loadingRuns && runs.length === 0 ? (
                  ...
                  <div className="p-12 text-center text-slate-400 font-mono text-xs space-y-3 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-slate-400">
                        点击「发起评测」按钮，系统将自动使用已就绪的题目测试证据全中率 (Recall@K) 与平均精度 (MAP@K)，并按单跳/多跳分桶。
                      </p>
                    </div>
                    {selectedTestSet.progress_done > 0 && (
                      <button
                  ...
                  <div className="space-y-4">
                    {runs.map((run, idx) => {
                      ...
                      const mapVal = run.metrics && mapKey in run.metrics ? run.metrics[mapKey] : null;
                      const coverageVal = run.metrics?.evidence_coverage;
                        ...
                        >
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-slate-200/80 text-xs font-mono">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {run.config.graph && (
                                <span className="px-1.5 py-0.5 rounded text-[10px] bg-indigo-100 text-indigo-700 font-mono">
                                  启用了知识图谱
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 sm:gap-3">
                              ...
                              {run.status === 'failed' && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleRerunFailedRun(run.id);
                                  }}
                                  disabled={rerunningRunId === run.id}
                                  className="flex items-center gap-1 px-2.5 py-1 rounded bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 text-xs font-semibold font-mono transition-colors cursor-pointer disabled:opacity-50"
                                  title="复位失败题并重跑（已成功的题跳过）"
                                >
                                  <RotateCw
                                    className={`w-3.5 h-3.5 text-rose-600 ${
                                      rerunningRunId === run.id ? 'animate-spin' : ''
                                    }`}
                                  />
                                  <span>{rerunningRunId === run.id ? '复位中…' : '重跑失败题'}</span>
                                </button>
                              )}
                          ...
                          {run.metrics && (
                            <div className="mt-3">
                              <div className="grid gap-2 text-xs font-mono grid-cols-2 sm:grid-cols-5">
                                ...
                                <div className="p-2.5 bg-white rounded-lg border border-slate-200/90 shadow-2xs">
                                  ...
                                  <span className="text-blue-700 font-bold text-sm">
                                    {mapVal !== null && mapVal !== undefined ? mapVal.toFixed(4) : '—'}
                                  </span>
                                </div>
                                ...
                                <div className="p-2.5 bg-white rounded-lg border border-slate-200/90 shadow-2xs">
                                  ...
                                  <span className="text-violet-700 font-bold text-sm">
                                    {coverageVal !== null && coverageVal !== undefined
                                      ? `${(coverageVal * 100).toFixed(1)}%`
                                      : '—'}
                                  </span>
                                </div>
      ...
      {isRunDetailModalOpen && selectedRunDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-4xl w-full p-6 shadow-2xl flex flex-col max-h-[88vh] space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <div className="flex items-center gap-2">
                {selectedRunDetail.status === 'failed' && (
                  <button
                    onClick={() => handleRerunFailedRun(selectedRunDetail.id)}
                    disabled={rerunningRunId === selectedRunDetail.id}
                    className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-mono font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-md transition-colors cursor-pointer disabled:opacity-50"
                    title="复位失败题并重跑（已有结果的题不会重复跑）"
                  >
                    <RotateCw
                      className={`w-3.5 h-3.5 text-rose-600 ${
                        rerunningRunId === selectedRunDetail.id ? 'animate-spin' : ''
                      }`}
                    />
                    <span>{rerunningRunId === selectedRunDetail.id ? '复位中…' : '重跑失败题'}</span>
                  </button>
                )}
                <button
            ...
            {selectedRunDetail.status === 'failed' && (
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs font-mono text-rose-700 flex items-center justify-between gap-3">
                <div className="flex items-start gap-2 min-w-0">
                  <AlertCircle className="w-4 h-4 shrink-0 text-rose-600 mt-0.5" />
                  <div className="truncate">
                    <span className="font-bold">评测异常中断:</span>{' '}
                    {selectedRunDetail.error_message || '部分题目在执行检索打分时遇到异常'}
                  </div>
                </div>
                <button
                  onClick={() => handleRerunFailedRun(selectedRunDetail.id)}
                  disabled={rerunningRunId === selectedRunDetail.id}
                  className="shrink-0 flex items-center gap-1.5 px-3 py-1 bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white rounded-lg text-xs font-semibold transition-colors cursor-pointer shadow-2xs disabled:opacity-50"
                >
                  <RotateCw
                  ...
                  />
                  <span>{rerunningRunId === selectedRunDetail.id ? '复位中…' : '立即重跑失败题'}</span>
                </button>
              </div>
            )}
            ...
            {selectedRunDetail.metrics && (() => {
              const k = selectedRunDetail.config?.top_k || 5;
              const recallKey = `recall@${k}`;
              const mapKey = `map@${k}`;
              const recallVal = selectedRunDetail.metrics[recallKey];
              const mapVal = selectedRunDetail.metrics[mapKey];
              const coverageVal = selectedRunDetail.metrics.evidence_coverage;

              return (
                <div className="grid gap-2.5 text-xs font-mono grid-cols-2 sm:grid-cols-5">
                  <div className="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl">
                    <span className="text-emerald-800 text-[10px] block mb-0.5">
                      Recall@{k} (证据全中)
                    </span>
                    <span className="text-emerald-700 font-bold text-lg">
                      {recallVal !== undefined && recallVal !== null
                        ? `${(Number(recallVal) * 100).toFixed(1)}%`
                        : '—'}
                    </span>
                  </div>
                  ...
                  <div className="p-3 bg-blue-50/70 border border-blue-200 rounded-xl">
                    ...
                    <span className="text-blue-700 font-bold text-lg">
                      {mapVal !== undefined && mapVal !== null
                        ? Number(mapVal).toFixed(4)
                        : '—'}
                    </span>
                  </div>

                  <div className="p-3 bg-violet-50/70 border border-violet-200 rounded-xl">
                    <span className="text-violet-800 text-[10px] block mb-0.5">
                      证据覆盖率 (Coverage)
                    </span>
                    <span className="text-violet-700 font-bold text-lg">
                      {coverageVal !== undefined && coverageVal !== null
                        ? `${(Number(coverageVal) * 100).toFixed(1)}%`
                        : '—'}
                    </span>
                  </div>
                  ...
                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    ...
                    <span className="text-slate-800 font-bold text-lg">
                      {selectedRunDetail.metrics.avg_latency_ms ?? 0} ms
                    </span>
                  </div>
            ...
            <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2 text-xs font-mono">
              <div className="flex items-center gap-1.5">
                ...
                >
                  未召回 ({selectedRunDetail.items ? selectedRunDetail.items.filter((i) => i.recall === 0).length : 0})
                </button>
                <button
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-1">
              {loadingRunDetail ? (
                ...
                selectedRunDetail.items
                  .filter((item) => {
                    if (runItemFilter === 'missed') return item.recall === 0;
                    if (runItemFilter === 'hit') return item.hit_rank !== null;
                    return true;
                  })
                  .map((item, idx) => {
                    const tsItem = (selectedTestSet?.items || []).find(
                      (t) => t.id === item.testset_item_id
                    );
                    const evidenceList = tsItem?.evidence || [];

                    return (
                      <div
                        key={item.testset_item_id || idx}
                        className={`p-3.5 rounded-xl border text-xs font-mono space-y-2.5 transition-colors ${
                          item.recall === 0
                      ...
                      >
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {/* Recall status：块级全中（后端 recall@k 同款口径） */}
                            {item.recall === 1 ? (
                            ...
                            {/* Hit Rank & Evidence Ranks */}
                            {item.evidence_ranks && item.evidence_ranks.length > 0 ? (
                              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-800 border border-blue-200" title="证据切片命中名次列表">
                                切片名次: [{item.evidence_ranks.join(', ')}]
                              </span>
                            ) : item.hit_rank !== null ? (
                            ...
                            {/* Evidence Coverage for multi-evidence items */}
                            {item.evidence_total !== undefined && item.evidence_total > 1 && (
                              <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-violet-100 text-violet-800 border border-violet-200">
                                证据覆盖: {item.evidence_matched ?? 0}/{item.evidence_total} ({Math.round(((item.evidence_coverage ?? (item.evidence_matched ? item.evidence_matched / item.evidence_total : 0)) * 100))}%)
                              </span>
                            )}

                            <span className="text-[11px] text-blue-700 font-semibold" title="单题平均精度 (Average Precision)">
                              MAP: {item.map !== null && item.map !== undefined ? Number(item.map).toFixed(3) : '0.000'}
                            </span>
                        ...
                        {evidenceList.length > 0 && (
                          <div className="text-[11px] text-slate-600 space-y-1 pt-1">
                            <div className="flex flex-wrap items-center gap-1.5">
                              <span className="text-slate-400 font-medium mr-0.5">考查证据:</span>
                              {evidenceList.map((ref, eIdx) => {
                                const evDocName =
                                  docMap[ref.document_id] ||
                                  `文档 ${ref.document_id.substring(0, 8)}...`;
                                return (
                                  <div
                                    key={`${ref.document_id}:${ref.chunk_index}`}
                                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded border text-[10px] bg-white border-purple-200 text-slate-700"
                                  >
                                    <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-purple-100 text-purple-800">
                                      证{eIdx + 1}
                                    </span>
      ...
      {/* Delete TestSet Custom Confirmation Modal */}
      {confirmDeleteSet && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h3 className="font-bold text-slate-900 text-base">删除评测集</h3>
                <p className="text-xs text-slate-500 leading-relaxed">
                  确定要彻底删除评测集「<span className="font-semibold text-slate-800">{confirmDeleteSet.name}</span>」吗？
                  此操作将永久清除该评测集的所有出题记录及评测运行历史，无法恢复。
                </p>
              </div>
            </div>
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                type="button"
                onClick={() => executeDeleteTestSet(confirmDeleteSet)}
                disabled={deletingId !== null}
              ...
              >
                {deletingId === confirmDeleteSet.id ? (
                  <>
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    <span>删除中...</span>
                  </>
