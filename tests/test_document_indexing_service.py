...
class _FakeRepository:
    ...
    # 签名与端口一字不差（keyword-only）：调用姿势漂移单测直接爆炸，
    # 不留给真实 adapter 在消费者循环里 TypeError 无限重投。
    async def delete_document(self, *, document_id: str) -> None:
        self.deletes.append({"document_id": document_id})
...
class IndexingServiceTests(TestCase):
    ...
    async def test_delete_calls_index_by_keyword(self) -> None:
        """delete 必须以 keyword-only 姿势打到 ChunkIndex（端口签名）——
        位置传参曾在真实 Neo4j adapter 上 TypeError，delete 任务无限重投。"""
        ...
        await service.delete("doc-9")

        self.assertEqual([{"document_id": "doc-9"}], repository.deletes)
