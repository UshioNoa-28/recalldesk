...
from app.application.services.search.query_rewrite_service import (
    QueryRewriteOutput,
    QueryRewriteService,
)
...
class QueryRewriteServiceTests(TestCase):
    async def test_entities_pass_through_and_prompt_asks_for_them(self) -> None:
        llm = AsyncMock()
        ...
        rewrite = await _service(llm).rewrite("苹果维C")

        self.assertEqual(["苹果", "维生素C"], rewrite.entities)  # 空白被剥掉
        prompt = llm.generate_structured.await_args.args[0]
        self.assertIn("entities", prompt)

    async def test_missing_entities_default_to_empty(self) -> None:
        llm = AsyncMock()
        llm.generate_structured = AsyncMock(
            return_value=QueryRewriteOutput(queries=["q1"], keywords=["k"])
        )

        rewrite = await _service(llm).rewrite("q")

        self.assertEqual([], rewrite.entities)
