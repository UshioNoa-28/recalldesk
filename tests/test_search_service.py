...
from app.application.services.search.search_service import SearchService, rrf_fuse
...
class _FakeGraphChannel:
    def __init__(self, hits: list[dict], error: Exception | None = None) -> None:
        self.hits = hits
        self.error = error
    ...
    async def recall(
        ...
        *,
        ...
        self.calls.append({"query": query, "entities": entities, "vector": query_vector})
...
class SearchServiceTests(TestCase):
    ...
    async def test_graph_channel_hits_join_fusion(self) -> None:
        """图频道命中作为第三路参与 RRF：独占候选也能进结果。"""

        dense = [_hit("p-0", 0.9)]
        graph = _FakeGraphChannel([_hit("p-9", 99.0, text="图谱捡回来的漏网之鱼")])
        repository = _FakeRepository(dense_hits=dense, sparse_hits=[])
        service = SearchService(
            index=repository, embedding_client=self.embedding, default_top_k=3, graph=graph
        )

        results = await service.search("缺铁吃什么")

        self.assertIn(9, [r["chunk_index"] for r in results])
        self.assertEqual(
            [{"query": "缺铁吃什么", "entities": [], "vector": [5.0] * 4}],
            graph.calls,
        )  # 原查询向量已递给链接兜底

    async def test_graph_channel_failure_degrades_to_plain_hybrid(self) -> None:
        """图挂了只是少一路：检索本身绝不因可选增强而 500。"""

        dense = [_hit("p-0", 0.9)]
        graph = _FakeGraphChannel([], error=ConnectionError("neo4j down"))
        repository = _FakeRepository(dense_hits=dense, sparse_hits=[])
        service = SearchService(
            index=repository, embedding_client=self.embedding, default_top_k=3, graph=graph
        )
        ...
        self.assertEqual([0], [r["chunk_index"] for r in results])

    def test_graph_enabled_property_reflects_channel_presence(self) -> None:
        repository = _FakeRepository(dense_hits=[], sparse_hits=[])
        plain = SearchService(index=repository, embedding_client=self.embedding, default_top_k=3)
        wired = SearchService(
            index=repository,
            embedding_client=self.embedding,
            default_top_k=3,
            graph=_FakeGraphChannel([]),
        )

        self.assertFalse(plain.graph_enabled)
        self.assertTrue(wired.graph_enabled)
