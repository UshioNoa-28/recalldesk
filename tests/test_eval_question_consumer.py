...
def _item(
    ...
    return EvalTestSetItem(
        id="item-1",
        status=status,
        evidence=evidence or [ChunkRef("doc-1", 0)],
    )


def _chunk(text: str = "片段内容", index: int = 0, name: str = "notes.md") -> dict:
    """检索索引里的那个分块：出题素材的唯一来源（原文件可重建它）。"""
...
class FakeChunkIndex:
    """类级配置：坐标不在 chunks 里 = 点已经不在了（文档被删或重建过）。"""

    chunks: ClassVar[dict[tuple[str, int], dict]] = {}
    ...
    async def get_chunk(self, *, document_id: str, chunk_index: int) -> dict | None:
        ...
        return type(self).chunks.get((document_id, chunk_index))
...
class FakeQuestionGenerator:
    ...
    async def generate(self, *, fragments: list) -> str:
        if self.error is not None:
            raise self.error
        for fragment in fragments:
            self.called_with.append((fragment.text, fragment.document_name))
        return "什么是 RAG？"
...
def _build_consumer(
    ...
    vectors: FakeChunkIndex | None = None,
    ...
    consumer = EvalQuestionConsumer(
        ...
        chunk_index=vectors or FakeChunkIndex(),
    )
...
class _RepoFixture(IsolatedAsyncioTestCase):
    ...
    def setUp(self) -> None:
        ...
        FakeChunkIndex.chunks = {("doc-1", 0): _chunk()}
        FakeChunkIndex.reads.clear()
...
class EvalQuestionConsumerHandleTaskTests(_RepoFixture):
    async def test_success_writes_item_marks_task_and_finalizes(self) -> None:
        ...
        self.assertEqual([("doc-1", 0)], FakeChunkIndex.reads)
    ...
    async def test_redelivery_of_finished_task_skips_llm(self) -> None:
        ...
        self.assertEqual([], FakeChunkIndex.reads)  # 素材也不用再读一次
    ...
    async def test_multi_evidence_item_fetches_all_chunks_and_combines(self) -> None:
        """多证据题逐坐标取料：主坐标在首逐块回查，全部分片喂给出题器。"""

        FakeEvalRepository.item = _item(
            evidence=[ChunkRef("doc-1", 0), ChunkRef("doc-2", 0)]
        )
        FakeChunkIndex.chunks = {
            ("doc-1", 0): _chunk("苹果富含维C", index=0, name="营养指南.txt"),
            ("doc-2", 0): _chunk("维C促进铁吸收", index=0, name="贫血手册.txt"),
        }
        generator = FakeQuestionGenerator()
        consumer, _ = _build_consumer(generator)
        ...
        self.assertEqual([("doc-1", 0), ("doc-2", 0)], FakeChunkIndex.reads)
        self.assertEqual(
            [
                ("苹果富含维C", "营养指南.txt"),
                ("维C促进铁吸收", "贫血手册.txt"),
            ],
            generator.called_with,
        )
        self.assertEqual(EvalTaskStatus.SUCCESS, FakeEvalTaskRepository.task.status)
        self.assertEqual(EvalItemStatus.READY, FakeEvalRepository.item.status)

    async def test_missing_secondary_evidence_is_terminal_failure(self) -> None:
        """次要证据缺失与主块同级：半截素材不出题，直接终态。"""
        ...
        self.assertEqual(EvalTaskStatus.FAILED, FakeEvalTaskRepository.task.status)
        self.assertEqual(EvalItemStatus.FAILED, FakeEvalRepository.item.status)
        error = FakeEvalRepository.item.error_message
        self.assertIn("取不到可出题的原文", error)
        self.assertIn("doc-2", error)  # 点名缺的是哪一块
        self.assertEqual(["ts-1"], self.evals().finalized)
