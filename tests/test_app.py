...
from app.api.controllers.eval import router as eval_router
from app.api.controllers.graph import router as graph_router
...
class AppAssemblyTests(TestCase):
    ...
    def test_document_routes_register_expected_paths(self) -> None:
    ...
    def test_graph_controller_registers_expected_paths(self) -> None:
        paths = {getattr(route, "path", None) for route in graph_router.routes}
        self.assertIn("/graph/entities/search", paths)
        self.assertIn("/graph/entities/{name_norm}/neighborhood", paths)
