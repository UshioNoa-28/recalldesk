...
class EvalTaskRepositoryTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        # 证据行带着 document_id 外键：没有这行文档就写不进题目坐标
        async with self._session_factory() as session:
    ...
    async def test_purging_the_document_takes_items_and_their_tasks(self) -> None:
        """0013 的删除通道：purge 题目 -> 连带未发出的出题任务级联消失。

        items 不再挂 documents 外键，DocumentService.delete 先调
        delete_items_referencing_document（题走 CASCADE 带走 eval_tasks），
        再删 documents 行。这里证明「任务确实跟着没了」——否则 publisher
        会给一道不存在的题出题。
        """
