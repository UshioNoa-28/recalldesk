...
以及 Document / Eval 出题 / Eval 逐题评测三个子类各自投递到独立的 stream / group。"""
...
class RedisEvalRunTaskQueueTests(IsolatedAsyncioTestCase):
    async def test_enqueue_sends_run_fields_to_run_stream(self) -> None:
        redis = AsyncMock()
        redis.xadd.return_value = "3-0"
        queue = RedisEvalRunTaskQueue(redis)
        ...
        self.assertEqual("3-0", message_id)
        redis.xadd.assert_awaited_once_with(
            name=settings.redis_eval_run_task_stream,
            fields={"task_id": "task-3", "run_id": "run-1", "item_id": "item-1"},
            maxlen=settings.redis_stream_maxlen,
            approximate=True,
        )
